...
<center>
    <div class="col-lg-6 d-flex justify-content-center align-items-center mt-4">
        <div class="card shadow-sm w-100 p-4 p-md-5" style="max-width: 64rem;">
            ...
            <form id="form" class="row g-3" {{-- method="POST" action="{{ route('dashboard.store.stage') }}" --}}
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="title:{{ $locale }}" id="title:{{ $locale }}" data-validation="required"
                        ...
                        value="{{ $stage->translate($locale)->title }}" placeholder="...">
                </div>
                @error('title:{{ $locale }}')
            ...
            <div class="col-6">
                ...
                <input type="text" name="description:{{ $locale }}" id="description:{{ $locale }}"
                    ...
                    value="{{$stage->translate($locale)->description}}" placeholder="...">
            </div>
            ...
            <div class="col-12">
                ...
                name="is_active" data-placeholder="Select" id="is_active" data-validation="required" data-validation-required="required">
                  <option></option>
                  <option value="0" {{(($stage->is_active == '0') ? 'selected' : '')}}>disactive</option>
                  <option value="1" {{(($stage->is_active == '1') ? 'selected' : '')}}>active</option>
                </select>
            </div>
            @error('is_active')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <div class="col-12">
                {{-- <img src="{{ $stage->image_path }}" class="avatar rounded me-2"
                                alt="profile-image" style="height:120px;width:150px"> --}}
                <div class="card">
                    <div class="card-body">
                        <h6>الصوره</h6>
                        <input type="file" data-default-file="{{ $stage->image_path }}" name="photo" class="dropify @error('photo') is-invalid @enderror">
                    </div>
                </div>
            </div>
            @error('photo')
            ...
            @enderror -->
                <div class="col-6">
                    <input type="text" name="stage_id" class="form-control form-control-lg d-none"
                        value="{{$stage->id}}">
                </div>
    ...
    $('#form').submit(function(e) {
        ...
        $.ajax({
            url: '{{route('dashboard.stage.update')}}',
            type: "POST",
            dataType: "json",
            data: formData,
            contentType: false,
