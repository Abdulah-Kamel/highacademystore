    ...
    <div class="col-lg-12 d-flex justify-content-center align-items-center">
        <div class="card shadow-sm w-100 p-4 p-md-5" style="max-width: 64rem;">
            ...
            <form id="form" class="row g-3"
                  {{-- method="POST" action="{{ route('dashboard.store.product') }}" --}} enctype="multipart/form-data">
                @csrf
                ...
                <div class="col-12">
                    <label class="form-label">اقصي كمية للطلب</label>
                    <input type="number" name="max_qty_for_order" id="max_qty_for_order"
                           class="form-control form-control-lg @error('max_qty_for_order') is-invalid @enderror"
                           value="{{ $product->max_qty_for_order }}" placeholder="...">
                </div>
                @error('quantity')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <div class="col-6">
                    ...
                    <select class="form-control show-tick ms select2" multiple
                            name="sizes[]" id="size">
                        <option value="" disabled>Select Your Option</option>
                        @if(is_array($product->sizes))
                            @foreach($sizes as $size)
                                <option value="{{$size}}" {{in_array($size,$product->sizes) ? 'selected' : '' }}>{{$size}}
                                </option>
                            @endforeach
                        @else
                            @foreach($sizes as $size)
                ...
                <div class="col-6">
                    ...
                    <select class="form-control show-tick ms select2" multiple
                            name="colors[]" id="color">
                        <option value="" disabled>Select Your Option</option>
                        @if(is_array($product->colors))
                            @foreach($colors as $color)
                                <option value="{{$color}}" {{in_array($color,$product->colors) ? 'selected' : '' }}>{{$color}}
                                </option>
                            @endforeach
                        @else
                            @foreach($colors as $color)
                ...
                <div class="col-12 text-center mt-4">
                    <button id="submit" type="submit"
                            class="btn btn-lg btn-block btn-dark lift text-uppercase">Update
                    </button>
                </div>
            </form>
