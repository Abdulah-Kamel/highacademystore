...
class VoucherController extends Controller
{
     public function index(Coupon $coupon)
    {
        ...
        return view('admin.voucher.index', compact('coupon'));
    }
  ...
  public function datatable(Coupon $coupon)
{
    ...
    if ($row->is_used) {
        ...
        ->addColumn('operation', function ($row) {
            $delete = '<a voucher_id=' . $row->id . ' class="btn btn-danger delete_btn">حذف</a>';
            $edit = '<a href="' . route('dashboard.vouchers.edit', $row->id) . '" class="btn btn-success">تعديل</a>';
...
public function add(Coupon $coupon)
    {
        return view('admin.voucher.create', compact('coupon'));
    }
